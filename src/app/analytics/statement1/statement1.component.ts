...
export class Statement1Component implements OnInit {
  ...
  // get_faculty_ia_details(){
  //   this.analyticsService.get_faculty_ia_details(this.fac_id,this.year,this.courseCode,this.section, this.deptId,this.terms).subscribe(res=>{
  //     this.iamark = res["iamark"]
  ...
  faculty_level(event: ChartSelectEvent) {
    ...
    console.log(this.faculty_data[ event.row]['courseCode'])
    this.analyticsService.get_faculty_ia_details(this.current_faculty, this.selectedyear, this.faculty_data[ event.row]['courseCode'], this.faculty_data[ event.row]['section'],
    this.faculty_data[ event.row]['deptId'], this.faculty_data[ event.row]['termNumber']).subscribe(res => {
      ...
      // this.ia_percent = r[0]['iaPercentage']
      // this.attendance_perc = r[0]['attendence'];
    })
  }
  ...
  getFacultyDetails(fac_id) {
    ...
    this.analyticsService.get_emp_subjects(this.current_faculty, this.selectedyear, this.terms).subscribe(res => {
      subs = res["iamarks"]
      console.log(subs)
      this.faculty_data = subs
    },
      err => { },
      () => {
        for (let s of subs) {
          console.log(s)
          data.push([s['courseCode'], s['iaPercentage'], s['attendence']])
        }
        if (data.length > 1) {
